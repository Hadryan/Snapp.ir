    ...
    socket.on('refreshSocket',function(params){
	    ...
	    repository.drivers.getSituation(params, function (result) {
            if(result.Result) {
	            ...
	            tLParams.description = "Driver socket become refresh.but accord database error. error was:"+JSON.stringify(result);
	            repository.user.setTripLog(tLParams, function (tLResult) {});
                socket.emit('refreshSocketRes', result);
            }
        });
	    socket.on('disconnect', function () {
		    redisClient.get(socket.id, function (error, userId) {
			    if(error){
				    tLParams.newSituation = null;
				    tLParams.description = "Driver become offline.but can not get his id from redis.error was:"+JSON.stringify(error);
				    repository.user.setTripLog(tLParams, function (tLResult) {});
			    }
			    if(userId) {
				    ...
				    repository.user.disconnectUserSocket({userId: userId}, function (disconnectUserSocketResult) {
					    if(disconnectUserSocketResult.Result){
						    tLParams.newSituation = 1;
						    tLParams.description = "Driver "+userId+" become offline.";
						    repository.user.setTripLog(tLParams, function (tLResult) {});
					    } else if(disconnectUserSocketResult.ErrorCode){
						    tLParams.newSituation = null;
						    tLParams.description = "Driver become offline.but his situation is greather than 3";
						    repository.user.setTripLog(tLParams, function (tLResult) {});
					    } else {
						    tLParams.newSituation = null;
						    tLParams.description = "Driver become offline.but can not offline in database.error was:"+JSON.stringify(error);
						    repository.user.setTripLog(tLParams, function (tLResult) {});
					    }
				    });
			    }
